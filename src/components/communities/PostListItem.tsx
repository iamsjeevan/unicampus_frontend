...
import { useAuth } from '@/contexts/AuthContext'; // Still needed for other actions like voting
...
const PostListItem: React.FC<PostListItemProps> = ({ post, index, showCommunity = false, onClick, onVoteChange, onDeletePost }) => {
  const { isAuthenticated, user } = useAuth(); // Keep for voting and future re-instatement of auth check
  ...
  // --- TEMPORARY FOR UI TESTING: Force show delete button ---
  const isAuthor = true; // This will make the delete option always visible for now
  // --- END TEMPORARY ---
      ...
      >
        <CardContent className="p-4">
          <div className="flex justify-between items-start">
            ...
            {/* Options Menu - always visible for UI test as isAuthor is true */}
            {isAuthor && (
                <DropdownMenu>
