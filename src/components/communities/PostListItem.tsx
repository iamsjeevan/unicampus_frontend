...
const PostListItem: React.FC<PostListItemProps> = ({ post, index, showCommunity = false, onClick, onVoteChange, onDeletePost }) => {
  ...
  const isAuthor = true; // This will make the delete option always visible for now. Revert to _isAuthor_original_check when auth is stable.
  // --- END TEMPORARY ---
  ...
  const handleDeletePostConfirm = async (e?: MouseEvent) => {
    ...
    // When real auth is back, uncomment and use _isAuthor_original_check:
    ...
    try {
        ...
        if (response.status === 'success') {
            toast({ title: "Post Deleted", description: `"${post.title}" has been successfully removed.` });
